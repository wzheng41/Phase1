...
def open_file(file_num):
    with open(r'./Z/' + str(file_num + 1) + '.csv', 'rt')as ff:
...
def normalize(lst):
    sqp = sum(lst)
    return map(lambda x: float(x) / sqp, lst)
            ...
            with open(completeName, mode='w') as file:
                for index in range(20):
                    ...
                    for i in range(0, len(wrd), s):
                        ...
                        if len(vector) == 3:
                            x.append(
                                '<<{0},{1},{2}>,{3},{4},{5}>'.format(f, index + 1, i, vector[0], vector[1], vector[2]))
                    writer.writerow(x)

    return x


def gaussian(x,sigma=0.25,u=0.0):   #use gaussian
    x=float(x-u)/sigma
    return math.exp(-x*x/2.0)/math.sqrt(2.0*math.pi)/sigma


def bands(r):    #create bands
    array=np.array([0.0]*2*r)
    for i in range(1,2*r+1):
        v1,e1=quad(gaussian,(i-r-1)/r,(i-r)/r)
        v2,e2=quad(gaussian,-1,1)
        length=2*(v1/v2)
        array[i-1]=length
    x=[]
    left=-1
...
def task2():
    ...
    wrd_freq = {}  # number of word appeared in dir
...
def plot_heatmap(text, temp, f, choice):
    ...
    plt.savefig("./Task3Output/{0}-{1}.png".format(f, choice))
...
if __name__ == '__main__':
    ...
    task1(3, 2)
    task2()
                       #test TF,TF_IDF,TF_IDF2 for the 6 files
